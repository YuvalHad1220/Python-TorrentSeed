...
class Torrent:
    ...
    client_id: int # the id of the client. The reason we save the id and not a reference to the object is because it is easier to both save and load, and neither the client or torrent has functions that work on each other 
...
def create_from_user_input(torrent_name, torrent_size, download_speed, upload_speed, announce_url, info_hash, client_id, downloaded, uploaded) -> Torrent:
    ...
    return Torrent(torrent_name, torrent_size, seeders, leechers, download_speed, upload_speed, is_start_announced, announce_url, info_hash, time_to_announce, client_id, downloaded, uploaded)
